...
const navigationMenuEase = "ease-[cubic-bezier(0.22,1,0.36,1)]"
const navigationMenuDuration = "duration-[0.35s]"

const navigationMenuTriggerStyle = cva([
  "group/navigation-menu-trigger inline-flex h-9 w-max items-center justify-center rounded-sm px-2.5 py-1.5",
...
const navigationMenuContentClassName = cn(
  "h-full w-auto p-1",
  `transition-[opacity,transform,translate] ${navigationMenuDuration} ${navigationMenuEase}`,
...
const navigationMenuPositionerClassName = cn(
  "isolate z-50 h-(--positioner-height) w-(--positioner-width) max-w-(--available-width)",
  `transition-[top,left,right,bottom] ${navigationMenuDuration} ${navigationMenuEase}`,
  "data-instant:transition-none",
  "data-[side=bottom]:before:top-[-10px] data-[side=bottom]:before:right-0 data-[side=bottom]:before:left-0",
)

const navigationMenuPopupClassName = cn(
  "relative h-(--popup-height) w-(--popup-width) xs:w-(--popup-width)",
  "origin-(--transform-origin) rounded-sm outline-none",
  "bg-popover text-popover-foreground shadow ring-1 ring-foreground/10",
  `transition-[opacity,transform,width,height,scale,translate] ${navigationMenuDuration} ${navigationMenuEase}`,
...
const navigationMenuLinkClassName = cn(
...
const navigationMenuIndicatorClassName = cn(
  "top-full z-1 flex h-1.5 items-end justify-center overflow-hidden",
  "data-[state=hidden]:animate-out data-[state=hidden]:fade-out",
  "data-[state=visible]:animate-in data-[state=visible]:fade-in",
)

const navigationMenuChevronClassName = cn(
...
function NavigationMenuTrigger({
  ...
  return (
    ...
    >
      {children}{" "}
      <ChevronDownIcon
        className={navigationMenuChevronClassName}
        aria-hidden="true"
      />
    </NavigationMenuPrimitive.Trigger>
  )
    ...
    <NavigationMenuPrimitive.Portal>
      ...
      >
        <NavigationMenuPrimitive.Popup
          className={cn(navigationMenuPopupClassName, popupClassName)}
        >
